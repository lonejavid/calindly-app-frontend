...
const TimeSelector = ({
  name,
  timeGap = 30,
  format = "12h", // Changed from "24h" to "12h"
  defaultValue,
  register,
  className,
  onSelect,
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        <div>
          <Input
            ...
            className={cn(
              `min-h-[46px] px-[14px]
             !py-[10px] !text-[15px] leading-[1.5] cursor-pointer 
            w-[110px] rounded-[8px] !h-auto focus-within:!ring-0`, // Made width slightly bigger for 12h format
              className && className
            )}
            placeholder="Time"
            value={defaultValue}
      ...
      <PopoverContent
        className="z-10 
        overflow-y-scroll w-[140px] max-h-[204px] mt-2
         bg-white border border-[##d4e0ed] rounded-[6px] p-[8px_0]"
        align="start"
        style={{
          boxShadow: "0 1px 5px rgba(0, 74, 16, 0.15)",
