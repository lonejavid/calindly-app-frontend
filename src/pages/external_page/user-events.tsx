      ...
      <div className="relative z-10 py-8 md:py-12 lg:py-16 px-4 sm:px-6">
        ...
        {isFetching ? (
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {/* Events Grid - Single grid container with equal height cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 lg:gap-10 max-w-6xl mx-auto mb-12 md:mb-16">
              {events?.map((event, index) => (
                <Link
                  key={index}
                  to={`/${username}/${event.slug}`}
                  className="group relative transform hover:scale-[1.03] transition-all duration-500 cursor-pointer h-full"
                >
                  {/* Card with fixed height */}
                  <div className="h-full bg-gradient-to-br from-slate-900/95 via-purple-900/90 to-indigo-900/95 backdrop-blur-2xl rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-10 border border-purple-400/50 hover:border-purple-300/80 transition-all duration-500 hover:shadow-2xl flex flex-col justify-between relative overflow-hidden ring-1 ring-purple-300/20 hover:ring-purple-300/40">
