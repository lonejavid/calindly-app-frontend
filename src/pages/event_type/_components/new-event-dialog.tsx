...
//         newErrors[`question_options_${q.id}`] = `Question ${index + 1} requires at least one option`;
//       }
      
//       // Check if options have empty values
//       if (['select', 'radio', 'checkbox'].includes(q.type)) {
//         const hasEmptyOptions = q.options.some(option => !option.trim());
//         if (hasEmptyOptions) {
//           newErrors[`question_options_${q.id}`] = `Question ${index + 1} has empty option values`;
//         }
//       }
//     });
...
//         // const { isConnected } = await checkIntegrationQueryFn(value as VideoConferencingPlatform);
//         const isConnected = true; // Mock response
//         if (!isConnected) {
//           setError(
//             `Google Meet is not connected. <a href="/integrations" target="_blank" class="underline text-blue-600 hover:text-blue-800">Visit the integration page</a> to connect your account.`
//           );
//           return;
//         }
//         setAppConnected(true);
...
//         // If changing type to text or textarea, clear options
//         if (field === 'type' && ['text', 'textarea'].includes(value as string)) {
//           updatedQuestion.options = [];
//         }
        
//         // If changing type to select, radio, or checkbox, initialize with empty options if none exist
//         if (field === 'type' && ['select', 'radio', 'checkbox'].includes(value as string) && q.options.length === 0) {
//           updatedQuestion.options = ['', '', ''];
//         }
...
//     // Clear related errors when updating question
//     if (errors[`question_${id}`] || errors[`question_options_${id}`]) {
//       setErrors(prev => ({ 
//         ...prev, 
//         [`question_${id}`]: null,
//         [`question_options_${id}`]: null
//       }));
//     }
//   };
...
//     alert('Event created successfully! Check console for data structure.');
