...
import { useBookingState } from "@/hooks/use-booking-state";
import { Fragment, useState, useMemo } from "react";
import { CheckIcon, ExternalLink, User, Calendar, Globe, ChevronDown, Clock } from "lucide-react";
import { scheduleMeetingMutationFn } from "@/lib/api";
import { toast } from "sonner";
import { Loader } from "@/components/loader";
    ...
    <div className="w-full flex-1 px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
      <div className="max-w-lg mx-auto lg:mx-0 lg:max-w-none">
        {isSuccess ? (
          ...
          <Fragment>
            {/* Event Title */}
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-3 sm:mb-4 text-gray-900 leading-tight break-words">
              {event.title}
            </h1>
            ...
            {/* Event Details Card */}
            <div className="bg-gray-50 rounded-lg p-4 sm:p-5 mb-6 sm:mb-8 space-y-3">
              {/* Duration */}
              <div className="flex items-center gap-3 text-sm sm:text-base text-gray-700">
                <div className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full bg-gray-200">
              ...
              <div className="flex items-center gap-3 text-sm sm:text-base text-gray-700">
                ...
                <span className="font-medium">
                  {event.locationType === 'GOOGLE_MEET_AND_CALENDAR' && 'Google Meet'}
                  {event.locationType === 'ZOOM_MEETING' && 'Zoom Meeting'}
                  {event.locationType === 'MICROSOFT_TEAMS' && 'Microsoft Teams'}
                  {!['GOOGLE_MEET_AND_CALENDAR', 'ZOOM_MEETING', 'MICROSOFT_TEAMS'].includes(event.locationType) && 'Video Conference'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 text-sm sm:text-base text-gray-700">
                ...
                <span className="font-medium">with {event.user.name}</span>
              </div>

              {/* Selected Date/Time - Show only if selected */}
              {selectedDate && selectedSlot && (
                <div className="flex items-start gap-3 text-sm sm:text-base text-gray-700 pt-2 border-t border-gray-200">
                  <div className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full bg-blue-100 mt-0.5">
                    <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600" />
                  </div>
                  <span className="font-medium break-words">
                    {format(parseISO(decodeURIComponent(selectedSlot)), "EEEE, MMMM d, yyyy 'at' h:mm a")}
                  </span>
                </div>
              )}
            </div>
