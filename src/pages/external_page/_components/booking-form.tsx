...
const BookingForm = (props: { eventId: string; duration: number; accessSpecifier: string }) => {
  ...
  const { mutate, isPending } = useMutation({ mutationFn: scheduleMeetingMutationFn });
  ...
  return (
    <div className="w-full max-w-lg mt-6 p-6 rounded-3xl shadow-2xl ring-1 ring-purple-300/20 border border-purple-400/30 backdrop-blur-xl bg-gradient-to-br from-slate-900/90 via-purple-900/80 to-indigo-900/90 text-white mx-auto relative z-10">
      {isSuccess ? (
        <div className="text-center pt-4">
          <h2 className="text-2xl flex items-center justify-center gap-2 font-bold mb-4 text-green-400">
            <span className="size-5 flex items-center justify-center rounded-full bg-green-700">
              <CheckIcon className="w-3 h-3 text-white" />
            </span>
            You are scheduled
          </h2>
          <p className="mb-4 text-slate-300">Your meeting has been scheduled successfully.</p>
          <p className="flex items-center text-sm justify-center gap-2 mb-4 text-purple-300">
            Copy link: <span className="font-normal">{meetLink}</span>
          </p>
          <a href={meetLink} target="_blank" rel="noopener noreferrer">
            <Button className="bg-gradient-to-r from-purple-600 via-pink-600 to-violet-600 text-white hover:from-purple-500 hover:to-violet-500">
        ...
        <Fragment>
          <h2 className="text-xl font-bold mb-6 text-white drop-shadow-lg">Enter Details</h2>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              {/* Name Field */}
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        className="bg-slate-800/60 text-white placeholder:text-slate-400 border border-purple-500/30 focus:ring-purple-500"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        className="bg-slate-800/60 text-white placeholder:text-slate-400 border border-purple-500/30 focus:ring-purple-500 min-h-32"
                        placeholder="Please share anything that will help prepare for our meeting."
                        {...field}
                      />
                    </FormControl>
              ...
              <Button
                disabled={isPending}
                type="submit"
                className="bg-gradient-to-r from-purple-600 via-pink-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white shadow-lg"
              >
                {isPending ? <Loader color="white" /> : "Schedule Meeting"}
              </Button>
            </form>
