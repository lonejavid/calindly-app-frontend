...
// Utility function to convert UTC time to local for display
...
const BookingForm = (props: { event: Event }) => {
  ...
  const onSubmit = (values: FormData) => {
    ...
    // Parse the selected slot as local time first
    const localStartTime = parseISO(decodedSlotDate);
    const localEndTime = addMinutes(localStartTime, event.duration);
    
    // Convert local times to UTC for backend
    const utcStartTime = convertLocalToUTC(localStartTime);
    const utcEndTime = convertLocalToUTC(localEndTime);

    console.log('Time conversion:', {
      selectedSlot: decodedSlotDate,
      ...
      timezoneOffset: localStartTime.getTimezoneOffset(),
    ...
    const payload = {
      ...
      ...(event.allowGuests && values.guestEmails && { guestEmails: values.guestEmails }),
      eventId: event.id,
      startTime: utcStartTime.toISOString(), // Send UTC time to backend
      endTime: utcEndTime.toISOString(), // Send UTC time to backend
      questionAnswers,
    };
    ...
    mutate(payload, {
      onSuccess: (response: any) => {
        setMeetLink(response.data.meetLink);
        setBookingDetails({
          startTime: localStartTime.toISOString(), // Store local time for display
          endTime: localEndTime.toISOString(), // Store local time for display
          guestName: values.guestName,
        });
        handleSuccess(true);
        setTimeout(() => {
  ...
  const formatBookingTime = () => {
    ...
    // bookingDetails already contains local time for display
    const startTime = new Date(bookingDetails.startTime);
    const endTime = new Date(bookingDetails.endTime);
          ...
          <div className="text-center space-y-4 sm:space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 sm:p-6 text-left space-y-3 sm:space-y-4">
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-gray-600">
                  <Globe className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span className="break-words">{event.timeZoneDisplay || "Local Time"}</span>
                </div>
              </div>
            </div>
