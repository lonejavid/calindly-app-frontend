  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Event Creation Demo</h1>
        <button
          onClick={() => setIsOpen(true)}
          className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center gap-2 mx-auto"
        >
...
interface CreateEventData {
  ...
  dateRangeLimit: number; // how far in future can people book (in days)
}
...
const EventCreationSidePanel = ({ isOpen, onClose }: EventCreationSidePanelProps) => {
  ...
  const [minimumNotice, setMinimumNotice] = useState<number>(4);
  const [noticeType, setNoticeType] = useState<'minutes' | 'hours' | 'days'>('hours');
  const [dateRangeLimit, setDateRangeLimit] = useState<number>(60);
  ...
  const validateForm = (): boolean => {
    ...
    if (dateRangeLimit < 1) newErrors.dateRangeLimit = 'Date range must be at least 1 day';
  ...
  // Helper function to convert notice to minutes
  const getMinimumNoticeInMinutes = (): number => {
    switch (noticeType) {
      case 'minutes':
        return minimumNotice;
      case 'hours':
        return minimumNotice * 60;
      case 'days':
        return minimumNotice * 24 * 60;
      default:
        return minimumNotice * 60;
    }
  };

  // Helper function to format notice display
  const formatNoticeDisplay = (): string => {
    const unit = noticeType === 'minutes' ? 'minute' : noticeType === 'hours' ? 'hour' : 'day';
    return `${minimumNotice} ${unit}${minimumNotice !== 1 ? 's' : ''}`;
  };
  ...
  const handleSubmit = (): void => {
    ...
    const eventData: CreateEventData = {
      ...
      isActive: true,
      // New availability fields
      minimumNotice: getMinimumNoticeInMinutes(),
      noticeType,
      dateRangeLimit,
    };
...
// const EventCreationSidePanel = ({ isOpen, onClose }: EventCreationSidePanelProps) => {
//   // Mock mutation since we don't have the real API functions
//   const isPending = false;
//   const handleMutate = (data: CreateEventData, callbacks: { onSuccess: () => void; onError: (error: Error) => void }) => {
